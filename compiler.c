...
static int resolveLocal(Compiler *compiler, Token *name)
{
    for (int i = compiler->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            if (local->depth == -1)
                error("Cannot read local variable in its own initailizer.");
            return i;
            }
    }
...
static inline void beginScope(void)
{
    current->scopeDepth++;
}

static inline void endScope(void)
{
    current->scopeDepth--;
...
ParseRule rules[] = {
    ...
    { NULL,     binary,  PREC_EQUALITY },   // TOKEN_GREATER
    { NULL,     binary,  PREC_EQUALITY },   // TOKEN_GREATER_EQUAL
...
static void classDeclaration(void)
{
    ...
    if (match(TOKEN_LESS)) {
        consume(TOKEN_IDENTIFIER, "Exepct superclass name.");
        variable(false);
